 ...
 * These helper functions enforce multi-layer guardrails on user input and AI output.
 */
...
/**
 * Check whether a user message contains blocked clinical or medical safety terms.
 * Partial matching ensures phrases like "pregnancy advice" are caught.
 */
  ...
  for (const keyword of BLOCKED_KEYWORDS) {
    if (normalized.includes(keyword.toLowerCase())) {
      return {
        ...
        safeReply: SAFE_BLOCKED_RESPONSE,
      };
    }
  }
...
/**
 * Validate Gemini response text before it is shown to the user.
 * This acts as a second safety layer after the system prompt.
 */
export function validateAiResponse(rawResponse: string) {
  ...
  for (const keyword of RESPONSE_BLOCKERS) {
    if (normalized.includes(keyword.toLowerCase())) {
      return {
        safe: false,
        safeText: SAFE_VALIDATION_FALLBACK,
